                                                           ...
                                                           jintArray pixels_, jint w, jint h)
{
   ...
   // Mat real_depth1(h, w * 2, CV_16UC1, cbuf);
    ...
    Rect rect(input1.cols * 0.15, 0, input1.cols*0.8, input1.rows);
    ...
    for(int i =0; i<h; i++)
    {
        for(int j = 0; j<w*2;j++)
        {
            int tem_depth = int(real_depth.at<uchar>(i,j));
            if(tem_depth > 3000)
                real_depth.at<uchar>(i,j) = 0;
        }
    }
    */
    Mat imggray;
  ...
  // multiply(real_depth, imggray_copy_threshold, depth_pic_top, 1, CV_32F);
   ...
   s_result = "length: "+to_string(cube_int[0]/10)+"."+to_string(cube_int[0]%10)+
           "cm width: " +to_string(cube_int[1]/10)+"."+to_string(cube_int[1]%10)+
           "cm height: "+to_string(cube_int[2]/10)+"."+to_string(cube_int[2]%10)+
           "cm volume: "+to_string(cube_int[3]/1000)+"."+to_string(cube_int[3]%1000);
